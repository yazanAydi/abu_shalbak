...
describe("POS login password-change recovery", () => {
  let container;
  let root;
  ...
  test("must-change login shows Arabic form then restores access after change", async () => {
    ...
    act(() => {
      setInput(inputs[0], "c1");
      setInput(inputs[1], "oldpass123");
    });
    ...
    act(() => {
      setInput(pwInputs[0], "oldpass123");
      setInput(pwInputs[1], "newpass123");
      setInput(pwInputs[2], "newpass123");
    });
