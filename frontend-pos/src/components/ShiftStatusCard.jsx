...
export default function ShiftStatusCard({ listRow, detail }) {
  ...
  // GET /api/shifts already returns expected_cash and sale_count per row, so the
  // card needs no per-shift detail request; `detail` stays supported for callers
  // that happen to have it already.
  const expected =
