...
/**
 * A PartyPicker value is a customer/supplier object — never a DOM/React event.
 * @param {unknown} value
 */
export function isPartyValue(value) {
 ...
 * Empty query returns a short browse list so the name field can be chosen on click.
 * @param {string} query
...
export async function searchPartiesApi(query, opts = {}) {
  ...
  const [custRes, supRes] = await Promise.all([
    api.get(`/api/customers${qs}`, { headers: getAuthHeaders() }),
    api.get(`/api/suppliers${qs}`, { headers: getAuthHeaders() }),
  ]);

  const customers = asRowList(custRes.data).map((c) => ({
    type: "customer",
    id: c.id,
    name: c.name,
  ...
  const suppliers = asRowList(supRes.data).map((s) => ({
    type: "supplier",
    id: s.id,
    name: s.name,
